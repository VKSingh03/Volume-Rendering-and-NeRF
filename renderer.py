...
class VolumeRenderer(torch.nn.Module):
    ...
    def _compute_weights(
        ...
        no_directions = rays_density.shape[0]
        no_points = rays_density.shape[1]
        
        # getting device
        device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')

        sigmas = [torch.ones((no_directions, 1)).to(device)] # for T = 1
        
        # Calculating 2nd term of T
        for point in range(no_points - 1):
            e = torch.exp(-rays_density[:, point] * deltas[:, point])

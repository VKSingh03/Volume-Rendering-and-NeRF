...
def render_images(
    ...
    for cam_idx, camera in enumerate(cameras):
        ...
        if cam_idx == 0 and file_prefix == '':
            xy_grid_image = vis_grid(xy_grid, image_size)
            plt.imsave(f'results/1_3_grid.png',xy_grid_image)

        # TODO (1.3): Visualize rays using vis_rays
        if cam_idx == 0 and file_prefix == '':
            bundle_rays_image = vis_rays(ray_bundle, image_size)
            plt.imsave(f'results/1_3_rays.png',bundle_rays_image)
        ...
        if cam_idx == 0 and file_prefix == '':
            ...
            render_points('results/1_4_points_sampling.jpg', points, image_size=256, color=[0.7, 0.7, 1], device=None)
        ...
        if cam_idx == 2 and file_prefix == '':
            plt.imsave(f'results/1_5_depth.png', out['depth'].view(image_size[1], image_size[0]).detach().cpu())
...
def render(
    ...
    imageio.mimsave('results/1_5_color_render.gif', [np.uint8(im * 255) for im in all_images], loop = 0)
...
def train(
    ...
    ##################################################################################################
    # modified for rounding the centre of box center and side lengths. 
    box_center = tuple(np.array(model.implicit_fn.sdf.center.data.detach().cpu()).tolist()[0])
    box_side_lengths = tuple(np.array(model.implicit_fn.sdf.side_lengths.data.detach().cpu()).tolist()[0])

    rounded_box_center = tuple(round(coord, 2) for coord in box_center)
    rounded_box_side_lengths = tuple(round(length, 2) for length in box_side_lengths)

    print("Rounded Box center:", rounded_box_center)
    print("Rounded Box side lengths:", rounded_box_side_lengths)
    ##################################################################################################
